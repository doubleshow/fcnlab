...
let SequenceView =  React.createClass({
    ...
    render(){
        ...
        let items = _.map(this.props.sequence, (step, i) =>{
            let duration = ''
            if (step.duration){
                duration = <span>({(step.duration/1000).toFixed(2)} sec)</span>
            }
            return <li className={ i == this.state.index ? 'current' : ''}>{step.command} {step.filename} {duration} </li>
        })
...
function generate(){
    ...
    _.last(sequence).duration = 10000
